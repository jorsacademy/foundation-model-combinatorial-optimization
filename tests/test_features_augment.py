...
def test_feature_shapes_and_masks() -> None:
    problem = generate_problem(GeneratorConfig(family="set_cover", variable_count=8, seed=8))
    graph = featurize(problem)
    assert graph.variable_features.shape == (8, len(VARIABLE_FEATURE_NAMES))
...
def test_equivalent_view_preserves_optimal_objective() -> None:
    problem = generate_problem(
        GeneratorConfig(family="independent_set", variable_count=8, seed=7)
    )
    view = equivalent_view(problem, np.random.default_rng(12))
    assert solve_exact(view).objective == solve_exact(problem).objective


def test_graph_embedding_is_invariant_to_equivalent_permutation_and_row_scaling() -> None:
    set_global_seed(3)
    problem = generate_problem(
    ...
    model.eval()
    with torch.no_grad():
        original_embedding = model.encode(featurize(problem), problem.family).graph_embedding
        view_embedding = model.encode(featurize(view), view.family).graph_embedding
    assert torch.allclose(original_embedding, view_embedding, atol=2e-5, rtol=1e-5)
