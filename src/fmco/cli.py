...
from fmco.benchmark import evaluate_model, save_report_csv, save_report_json
...
from fmco.experiment import ResearchConfig, run_research_experiment, save_research_report
from fmco.features import featurize, feature_schema
...
def build_parser() -> argparse.ArgumentParser:
    ...
    schema = subparsers.add_parser("schema", help="print the graph feature schema")
    schema.add_argument("--output", type=Path)

    generate = subparsers.add_parser("generate", help="generate one deterministic BLP instance")
    ...
    collect = subparsers.add_parser("collect", help="generate an exact-oracle labelled corpus")
    collect.add_argument("--families", choices=FAMILIES, nargs="+", required=True)
    ...
    pretrain = subparsers.add_parser("pretrain", help="self-supervise the shared encoder")
    ...
    adapt = subparsers.add_parser("adapt", help="train problem-family decision adapters")
    ...
    adapt.add_argument("--tasks", choices=FAMILIES, nargs="+", required=True)
    ...
    solve = subparsers.add_parser("solve", help="predict, repair, and audit one problem")
    ...
    benchmark = subparsers.add_parser("benchmark", help="evaluate one checkpoint on a corpus")
    ...
    research.add_argument("--pretrain-instances-per-family", type=int, default=32)
    research.add_argument("--train-instances-per-family", type=int, default=32)
    research.add_argument("--validation-instances-per-family", type=int, default=8)
    research.add_argument("--test-instances-per-family", type=int, default=12)
    research.add_argument("--transfer-pool-instances", type=int, default=32)
    research.add_argument("--transfer-validation-instances", type=int, default=8)
    ...
    research.add_argument("--transfer-shots", type=int, nargs="+", default=[4, 16])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "generate":
            ...
            _write_or_print({"output": str(args.output), "problem": problem.to_dict()}, None)
            return 0
        ...
        if args.command == "pretrain":
            ...
            model = FoundationCOModel(
                ModelConfig(hidden_dim=args.hidden_dim, rounds=args.rounds)
            )
        ...
        if args.command == "solve":
            ...
            with torch.no_grad():
                logits = model.decision_logits(graph, problem.family).cpu().numpy()
        ...
        if args.command == "benchmark":
            ...
            report = evaluate_model(model, corpus.records, model_label=args.label)
            if args.output_json:
